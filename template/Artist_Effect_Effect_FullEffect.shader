...
vec3 computeParticlePosition(in vec3 startVelocity, in vec3 lifeVelocity, in float age, in float normalizedAge, vec3 gravityVelocity, vec4 worldRotation, vec3 dragData)
{
    ...
    void main()
    {
        ...
        // Apply rotation to mesh vertex
        // Simplified version: apply rotation around Z axis
        float rotZ = a_StartRotation0.x;
        float c = cos(rotZ);
        float s = sin(rotZ);
        vec3 rotatedMeshPos = vec3(
            a_MeshPosition.x * c - a_MeshPosition.y * s,
            a_MeshPosition.x * s + a_MeshPosition.y * c,
            a_MeshPosition.z
        );

        // Scale and add to center
        center += rotationByQuaternions(u_SizeScale * rotatedMeshPos * size, worldRotation);
...
#include "Camera.glsl";
    ...
    void main()
    {
        ...
        // ========================================
#ifdef USEDISTORT0
        vec2 distortBaseUV = (u_Distort0UVMode == 0.0) ? v_Texcoord0.xy : screenUV;
        vec2 distortUV = v_Texcoord6.xy + (distortBaseUV * u_DistortTex0_ST.xy + u_DistortTex0_ST.zw);
        vec4 distortSample = texture2D(u_DistortTex0, distortUV);

        gl_FragColor = vec4(distortSample.xy, 0.0, 1.0);
        return;
#else
        ...
        vec3 spec = mix(vec3(0.0), vec3(1.0), smoothstep(-w, w * 2.0, snl + u_EffectMainLightDir.w - 1.0)) * step(0.0001, u_EffectMainLightDir.w);
