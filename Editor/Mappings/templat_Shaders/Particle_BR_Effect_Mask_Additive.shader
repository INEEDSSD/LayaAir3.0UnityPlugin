Shader3D Start
{
    ...
    uniformMap: {
        ...
        u_UVScrollTex: { type: Vector4, default: [0, 0, 0, 0] },
        u_UVScrollMask: { type: Vector4, default: [0, 0, 0, 0] },
    ...
    defines: {
        ...
        STRETCHEDBILLBOARD: { type: bool, default: false },
        HORIZONTALBILLBOARD: { type: bool, default: false },
        VERTICALBILLBOARD: { type: bool, default: false },
...
GLSL Start
#defineGLSL BR_Effect_Mask_AdditiveVS

    #define SHADER_NAME BR_Effect_Mask_AdditiveVS
    ...
    void main()
    {
        ...
        // NOTE: Do NOT apply TransformUV here!
        // Each texture needs its own tiling (u_TilingOffset for main, u_Mask_ST for mask)
        // Unity: _MainTex_ST=(1,0.1,0,0) vs _Mask_ST=(1,1,0,0) - different per texture
        // Raw UV is passed to FS, where per-texture TransformUV is applied
    }

#endGLSL

#defineGLSL BR_Effect_Mask_AdditiveFS

    #define SHADER_NAME BR_Effect_Mask_AdditiveFS
    ...
    // UV transform with Laya Y-flip convention
    vec2 TransformUV(vec2 texcoord, vec4 tilingOffset)
    {
        vec2 transTexcoord = vec2(texcoord.x, texcoord.y - 1.0) * tilingOffset.xy + vec2(tilingOffset.z, -tilingOffset.w);
        transTexcoord.y += 1.0;
    ...
    void main()
    {
        vec2 rawUV = v_TextureCoordinate;
        vec2 mainUV = TransformUV(rawUV, u_TilingOffset) - u_Time * u_UVScrollTex.xy;
        vec2 maskUV = TransformUV(rawUV, u_Mask_ST) - u_Time * u_UVScrollMask.xy;

        #ifdef ALBEDOTEXTURE
            vec4 mainTex = texture2D(u_AlbedoTexture, mainUV);
            #ifdef Gamma_u_AlbedoTexture
                mainTex = gammaToLinear(mainTex);
            #endif
        #else
        ...
        #ifdef MASKMAP
            vec4 maskTex = texture2D(u_Mask, maskUV);
        #else
            vec4 maskTex = vec4(1.0);
        #endif

        vec4 color = u_TintColor * v_Color * mainTex * maskTex * 2.0;
