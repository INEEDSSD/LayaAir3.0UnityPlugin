...
{
    type:Shader3D,
    name: "Particle_Sanguo_Sanguo_particle_distort_add",
    ...
    uniformMap:{
        ...
        u_Alpha: { type: "Float", default: 1.0, index: 6, alias: "全局透明度", catalog: "基本设置", range: [0, 1] },
        ...
        u_MinX: { type: "Float", default: -2000, index: 20, alias: "裁剪最小X", catalog: "裁剪设置", catalogOrder: 2, hidden: "!data.CLIP_AREA" },
        u_MaxX: { type: "Float", default: 2000, index: 21, alias: "裁剪最大X", catalog: "裁剪设置", hidden: "!data.CLIP_AREA" },
        u_MinY: { type: "Float", default: -2000, index: 22, alias: "裁剪最小Y", catalog: "裁剪设置", hidden: "!data.CLIP_AREA" },
        u_MaxY: { type: "Float", default: 2000, index: 23, alias: "裁剪最大Y", catalog: "裁剪设置", hidden: "!data.CLIP_AREA" },
    ...
    defines: {
        CLIP_AREA: { type: bool, default: false, alias: "裁剪区域开关", catalog: "裁剪设置", catalogOrder: 2 }
    },
    styles: {
    ...
    shaderPass:[
        {
            pipeline:"Forward",
            VS:Sanguo_particle_distort_addVS,
            FS:Sanguo_particle_distort_addFS,
            statefirst: true,
            renderState: {
                blend: "Enable",
                srcBlend: "One",
...
GLSL Start
#defineGLSL Sanguo_particle_distort_addVS

#define SHADER_NAME Sanguo_particle_distort_addVS
...
void main()
{
    ...
    if (normalizedAge < 1.0)
    {
        ...
        // Clipping: approximate local position = world position - particle system world origin
        // NOTE: This is an approximation. It assumes the particle system has no rotation or
        // non-uniform scale. For a rotated/scaled particle system, the true local position
        // would require inverse(u_WorldMat) * center, but the inverse matrix is not available
        // in the particle VS. For most use cases (particle systems at identity rotation),
        // this approximation is accurate.
        #ifdef CLIP_AREA
            v_LocalPos = center - u_WorldPosition;
        #endif
...
#defineGLSL Sanguo_particle_distort_addFS

#define SHADER_NAME Sanguo_particle_distort_addFS
...
void main()
{
    ...
    // baseRGB = vertexColor.rgb * color.rgb * mainCol.rgb * glow * distortCol.rgb
    // alphaFactor = vertexColor.a * color.a * mainCol.a * glow
    // emission = baseRGB * alphaFactor * mainCol.a
    ...
    // Clipping area
    // Unity uses v.vertex.xyz (local space mesh vertex).
    // Laya approximation: v_LocalPos = center - u_WorldPosition (see VS comment for limitations).
    #ifdef CLIP_AREA
        float clipAlpha = 1.0;
        clipAlpha *= step(u_MinX, v_LocalPos.x);
        clipAlpha *= step(v_LocalPos.x, u_MaxX);
        clipAlpha *= step(u_MinY, v_LocalPos.y);
        clipAlpha *= step(v_LocalPos.y, u_MaxY);
        finalColor.rgb *= clipAlpha;
    #endif

    finalColor.rgb *= u_Alpha;
