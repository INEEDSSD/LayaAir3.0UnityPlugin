...
namespace LayaExport
{
    ...
    public static class MeshSimplifier
    {
        ...
        public static Mesh SimplifyMesh(Mesh originalMesh, int targetVertexCount, float quality = 0.7f)
        {
            ...
            Debug.Log($"MeshSimplifier: 开始简化mesh '{originalMesh.name}'");
            Debug.Log($"  原始顶点数: {originalVertexCount}");
            Debug.Log($"  理论最小目标: {targetVertexCount}");
            Debug.Log($"  简化质量: {quality:F2}");
            Debug.Log($"  实际目标顶点数: {actualTargetCount} (允许超出理论值以保证质量)");

            #if UNITY_MESH_SIMPLIFIER
            // ⭐ 优先使用UnityMeshSimplifier（如果已安装）
            try
            {
                Debug.Log($"  使用UnityMeshSimplifier库（高质量QEM算法）");
                Mesh advancedMesh = UnityMeshSimplifierIntegration.SimplifyMeshAdvanced(
                    originalMesh, actualTargetCount, quality);
                Debug.Log($"  简化后顶点数: {advancedMesh.vertexCount}");
                return advancedMesh;
            }
        ...
        private static Mesh SimplifyByVertexSampling(Mesh originalMesh, int targetVertexCount)
        {
            ...
            Debug.Log($"  简化策略: 抽取三角形");
            Debug.Log($"  原始三角形数: {triangleCount}, 目标三角形数: {targetTriangleCount}");
            ...
            Debug.Log($"  最终: 顶点数={newMesh.vertexCount}, 三角形数={newMesh.triangles.Length / 3}");
